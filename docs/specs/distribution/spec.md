...
holds 11 JUnit test files, so `verify` both compiles/packages the
artifact and runs that test suite. A further push to the same branch must
...
`main`. The Maven coordinates `com.github.zrdj:java-predicates` resolve on
JitPack against this repository, `ZrdJ/java-predicates`.
...
- **THEN** a build for tag `X` is requested from JitPack at
  `jitpack.io/com/github/ZrdJ/java-predicates/X/build.log`
